...
const getConfigFromPkgJson = function (cwd) {
  try {
    return Promise.resolve(_catch(function () {
      ...
      stderr( // `Warn ${yellow(`no package.json found. Assuming a pkg.name of "${pkgName}".`)}`
      kleur.yellow(`${kleur.yellow().inverse('WARN')} no package.json found. Assuming a pkg.name of "${pkgName}".`));
      ...
      return Promise.resolve(getInput({
        ...
        return Promise.resolve(getOutput({
          ...
          return Promise.resolve(getEntries({
            ...
            options.multipleEntries = options.entries.length > 1; // to disable compress you can put in false or 0 but it's a string so our boolean checks won't work
...
const getSizeInfo = function (code, filename, raw) {
  try {
    return Promise.resolve(gzipSize(code)).then(function (_gzipSize) {
      ...
      let brotli; //wrap brotliSize in try/catch in case brotli is unavailable due to
      //lower node version
...
const removeScope = name => name.replace(/^@.*\//, ''); // Convert booleans and int define= values to literals.
...
function createConfig(options, entry, format, writeMeta) {
  ...
  let config = {
    ...
    outputOptions: {
      ...
      name: options.name,
      file: path.resolve(options.cwd, getMain({
        options,
        entry,
        format
      }))
